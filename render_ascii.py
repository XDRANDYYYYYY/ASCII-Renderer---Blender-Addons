...
def inten(g,z,w,h):
    ...
    for i in range(w*h):
        if z[i]==float('inf'):
            continue
        c[i]=LIGHT_INT[max(1, min(9,1+int(max(0.0,min((g[i]-a[int(0.05*(len(a)-1))])/(a[int(0.95*(len(a)-1))]-a[int(0.05*(len(a)-1))]),1.0))**0.5*(9)+0.5)))]
    return c
def render(context,w,h):
    ...
    for obj in context.scene.objects:
        ...
        pic=None
        color=None
        if tem.active_material is None:
            color=(0,0,0)
        else:
            if tem.active_material.use_nodes:
                node=None
                for i in tem.active_material.node_tree.nodes:
                    if i.type=='BSDF_PRINCIPLED':
                        node=i
                        break
                if node is not None and node.inputs.get('Base Color').is_linked and node.inputs.get('Base Color').links[0].from_node.type=='TEX_IMAGE':
                    pic=node.inputs.get('Base Color').links[0].from_node.image
            else:
                    color=node.inputs.get('Base Color').default_value[:3]
            else:
                color=tem.active_material.diffuse_color[:3]
        for p in m.polygons:
